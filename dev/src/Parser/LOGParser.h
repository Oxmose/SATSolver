...
class LOGParser : public IParser
{
    ...
    private:
	    std::vector<Expr*> tseitinTransform(Expr *exp, unsigned int &p_maxIndex, std::map<std::pair<int, int>, Expr*> &corresp, std::map<std::pair<int, int>, Expr*> &ncorresp, std::map<std::string, function_s*> &funcorresp, std::map<std::string, unsigned int> &arrities, std::map<std::string, args_s*> &argscorresp);
